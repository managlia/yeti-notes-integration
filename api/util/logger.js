...
const abcde = (elem, f, l, file) => {
    const locationString = getLocationString(f, l, file);
    const logColor = getChalkColor(locationString);
    console.log(logColor(`${S_DEBUG}${locationString}${elem}`));
};

const errro = (elem, f, l, file) => {
    const locationString = getLocationString(f, l, file);
    const logColor = chalk.red;
    console.log(logColor(`${S_DEBUG}${locationString}${elem}`));
};

const getLocationString = (aFunction, aLine, aFile) => {
    let fn = '';
